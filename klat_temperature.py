...
def debyeT(vs):
    return (hpr.hbar /hpr.kB) * (6*pi / hpr.atmV)**(1/3) * vs
...
def kL_umklapp_PD_vs_T(param, avgV, N, vs, stoich, og, subst, nfreq, c, T):
    ...
    avgM = sum((1-c) * og['mass'] + c * subst['mass']) / sum(stoich)
    debf = debyeT(vs) * (hpr.kB / hpr.hbar)
    dfreq = debf / nfreq
    kL = 0
    for freq in np.arange(dfreq, debf, dfreq):
        ...
        kL = kL + (1/3) * spectral_C(vs, freq, T) * vs**2 * tau
    return kL
...
def likelihood(param, D):
    dA = D['At'] - feval_klat(param, D['Tt'], D)
...
if __name__ == '__main__':
        ...
        fil.close()
        D['Tt'], D['At'], D['Et'], D['It'] = \
        read_data('/Users/ramyagurunathan/Documents/PhDProjects/Fe2VAl/Fe2VAl/Data/Cobalt', 6, '00')
        ...
        D['s'] = [0.1, 0.5, 0.1] 
        D['locV'] = [1, 2, 1] #centers of distributions
        D['scaleV'] = [1, 1, 1] #std. deviation of distributions
        D['dim'] = len(D['distV'])
        
        D['pname'] = ['A', 'gruneisen', 'B']
        D['pname_plt'] = ['A', r'\gamma', 'B'] 
